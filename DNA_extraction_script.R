...
# Load data
df_dna <- read_csv("data/baobab_DNA.csv") |>
  clean_names() |> 
  select(-sample_id, -od1, -od2) |>
  filter(pvp != "PST") |>
  dplyr::rename("od_280" = "od_260_a280",
         "od_230" = "od_260_a230")

df_dna <- df_dna |>
  mutate(category_280 = case_when(
  od_280 < 1.78 ~ "Low",
  od_280 >= 1.78 & od_280 <= 1.95 ~ "Within",
  od_280 > 1.95 ~ "High"
  ))

df_dna <- df_dna |>
  mutate(category_260 = case_when(
    od_230 < 2.0 ~ "Low",
    od_230 >= 2.0 & od_230 <= 2.2 ~ "Within",
    od_230 > 2.2 ~ "High"
)) |>
  filter(temp != "Cold") |>
